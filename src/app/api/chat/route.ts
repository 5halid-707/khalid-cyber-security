...
- إذا سُئلت عن الخبرة العملية أو التقارير الأمنية، اذكر بفخر: تقرير اختبار اختراق Active Directory + pfSense شامل (48 خطوة، 5 ثغرات مكتشفة، 100% معالجة، إعادة اختبار ناجحة). التقرير متاح للتحميل في قسم "التقارير الأمنية" على الموقع. يوثّق: إعداد pfSense Firewall (ZFS + DHCP + قواعد)، Windows Server 2025 + Active Directory (Domain Controller + OUs + GPO)، اختبار اختراق كامل (Nmap + Impacket + Responder + BloodHound + Hashcat)، اكتشاف ثغرات (SMB Null Session + LLMNR Poisoning + GPP Passwords)، ومعالجة كاملة (GPO + Registry Hardening + Re-Testing).
...
function getOfflineReply(message: string, isAr: boolean): string {
  ...
  if (isAr) {
    ...
    if (has(["تقرير", "reports", "report", "اختبار اختراق", "penetration", "pfsense", "active directory", "اختراق ad", "ثغرات", "مختبر"])) {
      return pick([
        "خالد عنده تقرير أمني احترافي قابل للتحميل في موقعنا 📄🔧:\n\n🔍 تقرير اختبار اختراق Active Directory + pfSense\n📊 48 خطوة فحص كاملة\n🚨 5 ثغرات مكتشفة\n✅ 100% معالجة + إعادة اختبار ناجحة\n\nالأدوات المستخدمة: Kali Linux, Nmap, Impacket, Hashcat, Responder, BloodHound, Wireshark, pfSense, Windows Server 2025, GPO.\n\nالثغرات المكتشفة: SMB Null Session + LLMNR Poisoning + GPP Passwords + LDAP RootDSE + Credential Interception.\n\nالتقرير موثّق بالصور والأوامر الفعلية — شفافية كاملة! 📥 حمّله من قسم 'التقارير الأمنية' في الموقع.",
        "أحد أهم أعمال خالد العملية: تقرير اختبار اختراق AD شامل 🏆\n\nالمراحل:\n1️⃣ إعداد pfSense Firewall (ZFS + DHCP + قواعد)\n2️⃣ Windows Server 2025 + Active Directory (DC + OUs + GPO)\n3️⃣ اختبار اختراق: Nmap + Impacket + Responder + BloodHound + Hashcat\n4️⃣ التحصين: إغلاق كل الثغرات + Re-Testing\n\nالنتيجة: 48 خطوة موثّقة + 5 ثغرات + 100% معالجة. هذا ليس نظري — عمل فعلي!\n📥 التقرير متاح للتحميل في قسم 'التقارير الأمنية'.",
        "سؤال ممتاز عن التقارير! 📊\n\nخالد يقدم تقرير اختبار اختراق Active Directory + pfSense — مستوى Enterprise. التقرير يوثّق:\n• إعداد pfSense كاملاً (ZFS, WAN/LAN, DHCP, Firewall Rules)\n• Windows Server 2025 + AD (Domain Controller, 5 مستخدمين, GPO)\n• 37 خطوة فحص أمني (Nmap, BloodHound, Responder, Hashcat)\n• اكتشاف 5 ثغرات + معالجتها + Re-Testing\n\nالتقرير ثنائي اللغة (عربي/إنجليزي) — 48 خطوة بالصور. 📥 حمّله من الموقع!",
      ]);
    }
