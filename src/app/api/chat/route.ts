...
- إذا سُئلت عن الخبرة العملية أو التقارير الأمنية أو حماية الشبكات/المواقع/التطبيقات/قاعدة البيانات، اذكر بفخر: تقرير الحماية واختبار الاختراق وإغلاق الثغرات (53 خطوة موثّقة، 5 ثغرات مكتشفة، 100% إغلاق، إعادة اختبار ناجحة). التقرير يحمي: شبكاتك (pfSense Firewall)، مواقعك وتطبيقاتك الإلكترونية (Windows Server 2025 + Active Directory)، قاعدة بياناتك (منع اعتراض بيانات الاعتماد + حظر الوصول غير المصرّح). يغطي تحليل اختبارات اختراق لاكتشاف الثغرات بالأدوات: Kali Linux, Nmap, Impacket, Hashcat, Responder, BloodHound, Wireshark. الثغرات المكتشفة: SMB Null Session + LLMNR Poisoning + GPP Passwords + LDAP RootDSE + Credential Interception. الإغلاق: GPO + Registry Hardening + Re-Testing. مناسب لجميع الفئات: الأفراد، المؤسسات الصغيرة (1-50 موظف)، المتوسطة (50-250)، الكبيرة (250+). التقرير متاح للتحميل في قسم "التقارير الأمنية" على الموقع.
...
function getOfflineReply(message: string, isAr: boolean): string {
  ...
  if (isAr) {
    ...
    if (has(["تقرير", "reports", "report", "اختبار اختراق", "penetration", "pfsense", "active directory", "اختراق ad", "ثغرات", "مختبر", "حماية شبكة", "حماية موقع", "حماية تطبيق", "قاعدة بيانات"])) {
      return pick([
